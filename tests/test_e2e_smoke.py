...
async def test_full_happy_path(mock_geo, app, tmp_path):
    ...
    # 3. Admin gets the review email — subject is "[WTFDC] new signup: Ian"
    ...
    # 5. Welcome email landed — subject is "Welcome to WTFDC, Ian"
    ...
    assert out["sent"] == 1
    # Digest subject: "WTFDC for Ian — {date}"
    digests = [e for e in fake_email.sent
               if "WTFDC" in e["subject"] or "Good morning" in e["text"]]
    assert digests
    digest = digests[-1]
